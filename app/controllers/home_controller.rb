class HomeController < ApplicationController
  ...
  def get_jqgrid
    ...
    json_data = {
		...
		:total=>3,
		:records=>"6", 
		:rows=> JSON.parse(@exams.to_json(:only => [ :accession,:mrn,,:current_status,:code,:description,:modality,:resource_name,:current_status,:updated_at,:patient_class,:patient_type,:patient_location_at_exam,:radiology_department,:ordering_provider,:scheduler,:technologist]))    
    }
